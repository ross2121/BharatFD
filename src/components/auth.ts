...
try {
    ...
    const token = jwt.sign({ id: user.id},tokens, { expiresIn: "365d" });
...
export const  generaotp=async(req:any,res:any,next:any)=>{
...
if(reason=="FORGOTPASSWORD"){
    ...
    try{tranporter.sendMail(verifyotp,(err:any)=>{
        if(err){
            res.send(err);
        }
        else{
...
export const verifyotp = async (req:any, res:any, next:any) => {
    try {
        ...
        if (parseInt(code) === parseInt(req.app.locals.OTP)) {
            ...
            const token = jwt.sign({ id: newUser.id, role: "admin" },tokens, { expiresIn: "365d" });
...
export const createResetSession=async(req:any,res:any,next:any)=>{
